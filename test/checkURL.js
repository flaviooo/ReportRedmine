...
let endpointsCSEA = [
 ...
 // 'https://dataentryrifiuti.csea.it/DataEntryRifiuti/login.html',
    ...
    // 'http://192.168.40.112:8080/flussiBanca/movimenti/display.action',
    // 'http://192.168.40.114:8080/flussiBanca/movimenti/display.action',
    // 'http://csea-ng-mi:8080/Gestionale',
    ...
    // 'http://192.168.40.68/BonusGas/webapp/toLogin; http://192.168.40.67/BonusGas/webapp/toLogin nuovo',
    // 'https://csea.soluzionipa.it/admin/pagamenti/dati_ente',
    // 'http://piuma.csea.local:8080/share/page',
    // 'http://192.168.0.77/avcp_2017/',
    // 'https://serviziweb.inaz.it/portalecsea/default.aspx',
    // 'https://analytics.google.com/analytics/web/'
];

//async function getTimeEntriesXML() {
function getStatuServerCSEA() {
    //axios.get('https://www.csea.it', configCsea)
    Promise.all(endpointsCSEA.map((endpoint) => axios.get(endpoint, configCsea)))
        .then(
        /*
         (response) => {
...
async function doRequests(urls) {
    ...
    responses.forEach(resp => {
        ...
        try {
          ...
          console.log(" resp "+error.status);    
        } catch (error) {
            console.log(" respo "+error.status);    
        }
