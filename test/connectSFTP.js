...
class SFTPClient {
    ...
    async connect() {
        ...
        try {
            await this.client.connect(config_CDLAN.connSettings);
        } catch (err) {
    ...
    async getlastFiles(remoteDir, fileGlob) {
        console.log(`Listing ${remoteDir} ...`);
        let fileObjects;
        try {
            fileObjects = await this.client.list(remoteDir, fileGlob);
...
//    await client.listFiles(config_CDLAN.connSettings.remotePath);
    ...
    await client.downloadFile(
        config_CDLAN.dump.remotePath + "/" + pathLastDump, 
        config_CDLAN.dump.localPath+pathLastDump || "./download.zip");
