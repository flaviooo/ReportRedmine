...
exports.call = (req, res, next) => {
  ...
  let s = " SELECT  `issues`.`id`,  au.`login` as Autore, ass.`login`as Assegnato,timestampdiff(DAY, `issues`.`updated_on`, now()) as GiorniTrascorsi, " +
    ...
    //" AND ((issues.status_id IN ('"+idTipologia+"')) ) "+
  ...
  local_db.query(s, (err, result, fields) => {
    if (err) {
      ...
      // res.json(result);
     // console.log("Result view: "+ JSON.stringify(result));
      rows = result;
     // res.render('consultaRapid', { title: "Consulta" });
     //   res.render('consultaRapid', { title: "Consulta 1", results : JSON.stringify(result)});
     res.render('consultaRapid', { title: "Consulta 1", results : result});
...
exports.view = (req, res, next) => {
  ...
  local_db.query(s, (err, result, fields) => {
     ...
     */ 
     // res.render('consultaRapid', { result: rows} );
     // res.render('consultaRapid', { results: JSON.stringify(rows), locals: { results: JSON.stringify(rows) } });
     // res.render('consultaRapid', { title: "Consulta", results : JSON.stringify(result)});
     console.log("Result view: "+ JSON.stringify(result));
 //     res.json(results);
 res.render('consultaRapid', { title: "Consulta 1", results : result});
    
 // res.render('consultaRapid', { title: "Consulta", results: rows });  
